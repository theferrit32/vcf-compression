...
def measure():
    ...
    for end_pos in range(min_pos, max_pos, step):
        ...
        cmd = '/home/krferrit/vcf-compression/main_release sparse-query {filename} {ref} {start} {end}'.format(
            filename=sparse_filename, ref=reference_name, start=min_pos, end=end_pos)
        print(cmd)
    ...
    for start_pos in range(max_pos, min_pos, -step):
        ...
        #     sparse_filename, reference_name, min_pos, end_pos)
        cmd = '/home/krferrit/vcf-compression/main_release sparse-query {filename} {ref} {start} {end}'.format(
            filename=sparse_filename, ref=reference_name, start=start_pos, end=max_pos)
        print(cmd)
        start_time = time.time()
        proc = subprocess.Popen(cmd.split(), stdin=None, stdout=subprocess.DEVNULL)
        proc.wait()
    ...
    for start_pos in range(mid_pos, 0, -int(step/2)):
        ...
        cmd = '/home/krferrit/vcf-compression/main_release sparse-query {filename} {ref} {start} {end}'.format(
            filename=sparse_filename, ref=reference_name, start=start_pos, end=end_pos)
        print(cmd)
        start_time = time.time()
        proc = subprocess.Popen(cmd.split(), stdin=None, stdout=subprocess.DEVNULL)
        proc.wait()
    ...
    for end_pos in range(min_pos, max_pos, step):
        if iter >= iter_max:
            break
        iter += 1
        cmd = '/home/krferrit/vcf-compression/tabix {filename} {ref}:{start}-{end}'.format(
            filename=bgzip_filename, ref=reference_name, start=min_pos, end=end_pos)
        print(cmd)
        start_time = time.time()
        proc = subprocess.Popen(cmd.split(), stdin=None, stdout=subprocess.DEVNULL)
        proc.wait()
    ...
    for start_pos in range(max_pos, min_pos, -step):
        ...
        #     sparse_filename, reference_name, min_pos, end_pos)
        cmd = '/home/krferrit/vcf-compression/tabix {filename} {ref} {start} {end}'.format(
            filename=bgzip_filename, ref=reference_name, start=start_pos, end=max_pos)
        print(cmd)
        start_time = time.time()
        proc = subprocess.Popen(cmd.split(), stdin=None, stdout=subprocess.DEVNULL)
        proc.wait()
    ...
    for start_pos in range(mid_pos, 0, -int(step/2)):
        ...
        end_pos = (mid_pos-start_pos) + mid_pos
        cmd = '/home/krferrit/vcf-compression/tabix {filename} {ref} {start} {end}'.format(
            filename=bgzip_filename, ref=reference_name, start=start_pos, end=mid_pos)
        print(cmd)
        start_time = time.time()
        proc = subprocess.Popen(cmd.split(), stdin=None, stdout=subprocess.DEVNULL)
        proc.wait()
